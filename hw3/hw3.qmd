...
#create symlink in hw3 which points to the parquet
ln -s ~/Downloads/mimic-iv-3.1/hosp/labevents_pq ./labevents_pq

# Verify it works
ls -la labevents_pq
#confirmed: shows part-0.parquet
...
patients   <- read_csv(file.path(mimic_path, "patients.csv.gz"))
admissions <- read_csv(file.path(mimic_path, "admissions.csv.gz"))
transfers  <- read_csv(file.path(mimic_path, "transfers.csv.gz"))
procedures <- read_csv(file.path(mimic_path, "procedures_icd.csv.gz"))
diagnoses  <- read_csv(file.path(mimic_path, "diagnoses_icd.csv.gz"))
d_proc     <- read_csv(file.path(mimic_path, "d_icd_procedures.csv.gz"))
d_diag     <- read_csv(file.path(mimic_path, "d_icd_diagnoses.csv.gz"))

labs <- open_dataset("labevents_pq")
...
#Lab events (filtering before collecting bc large data)
lab_events <- labs %>%
  filter(subject_id == subject_id_input) %>%
  select(charttime) %>%
  collect() %>%
...
ggplot() +
  ...
  scale_size_manual(values = c(0.7, 2), guide = "none") +
