...
# Graph 1: Histogram of ICU stays distribution
...
-   **admission minute (anything unusual?)**
    ...
    -   This pattern of 0, 15, 30, and 45 seems to likely have been used as a system default or sort of rounding.
...
dir.exists("chartevents_pq")

# List any files in it
list.files("chartevents_pq")

# Define the vitals we are interested in
vital_items <- c(
  "220045" = "heart_rate",
  "220179" = "systolic_bp",
  "220180" = "diastolic_bp",
...
vital_itemids <- as.integer(names(vital_items))

# Start an in-memory DuckDB connection
con <- dbConnect(duckdb())

# Align DuckDB's working directory with R's
dbExecute(con, paste0("SET home_directory='", getwd(), "'"))

# Register ICU stays table in DuckDB
duckdb_register(con, "icu", icustays_tble |> select(subject_id, stay_id, intime, outtime))

# Create a view pointing to the Parquet folder for chartevents
...
# Query to get the first measurement within ICU stay
chartevents_tble <- dbGetQuery(con, "
  ...
  with_values AS (
    ...
    INNER JOIN chartevents_raw c
      ON fv.subject_id = c.subject_id
     AND fv.itemid = c.itemid
     AND fv.first_storetime = c.storetime
  )
  PIVOT with_values
